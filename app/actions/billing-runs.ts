...
const createDraftSchema = z.object({ apartment_id: z.string().uuid(), billing_year: z.coerce.number().int(), billing_month: z.coerce.number().int() });
...
export async function createBillingRunDraft(formData: FormData) {
  ...
  const { data: settings } = await supabase.from('app_settings').select('default_cutoff_day').single();
  const cutoffDay = settings?.default_cutoff_day ?? 3;
  const cutoffDate = format(computeCutoffDate(parsed.billing_year, parsed.billing_month, cutoffDay), 'yyyy-MM-dd');
  ...
  if (!rentRow) return { error: 'Missing rent price for selected period.' };
  ...
  const { data, error } = await supabase.from('billing_runs').insert({
    ...
    rent_amount_eur: Number(rentRow.amount_eur).toFixed(2),
    utilities_total_eur: utilities.toFixed(2),
    total_eur: (Number(rentRow.amount_eur) + utilities).toFixed(2),
    is_locked: false
  }).select('id').single();

  if (error) {
    if (error.code === '23505') return { error: 'Billing run already exists for this apartment and period.' };
...
export async function lockBillingRun(formData: FormData) {
  ...
  const supabase = createClient();
  const { data, error } = await supabase.rpc('lock_billing_run', { p_run_id: parsed.id });
...
export async function toggleBillingRunPaid(formData: FormData) {
  const parsed = paidSchema.parse({ id: formData.get('id'), is_paid: formData.get('is_paid') === 'true' });
  const supabase = createClient();

  const { data: run } = await supabase.from('billing_runs').select('is_locked').eq('id', parsed.id).single();
  if (!run?.is_locked) return { error: 'Only locked runs can be marked paid.' };

  const { error } = await supabase
    .from('billing_runs')
    .update({ is_paid: parsed.is_paid, paid_at: parsed.is_paid ? new Date().toISOString() : null })
    .eq('id', parsed.id);
  if (error) return { error: error.message };
  revalidatePath('/app/dashboard');
