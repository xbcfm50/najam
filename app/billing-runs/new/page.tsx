...
export default async function NewBillingRunPage({
  ...
  searchParams: { apartment?: string; year?: string; month?: string };
}) {
  ...
  const utilitiesTotal = (previewBills ?? []).reduce(
    (sum, bill: { amount_eur: string | number }) => sum + Number(bill.amount_eur),
    0
  );
    ...
    <div className="space-y-6">
      ...
      <section className="rounded border bg-white p-4 space-y-2">
        <form className="grid gap-2 md:grid-cols-4" method="get">
          ...
          <input
            ...
            className="rounded border p-2"
          />
          <button className="rounded border px-3 py-2">Preview</button>
        </form>
        <div>Apartment: {apartments?.find((a) => a.id === apartmentId)?.name ?? 'N/A'}</div>
        <div>
          Billing Period: {year}-{String(month).padStart(2, '0')}
        </div>
          ...
          <tbody>
            {(previewBills ?? []).map((bill: any) => (
              <tr key={bill.id} className="border-t">
                <td>{bill.received_on}</td>
                <td>{bill.utility_types?.name}</td>
                <td>
                  {bill.period_year}-{String(bill.period_month).padStart(2, '0')}
                </td>
                <td>{formatEur(bill.amount_eur)}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {!existingDraft ? (
        <form action={createBillingRunDraft}>
          ...
          <button
            disabled={!rentPrice || !apartmentId}
        ...
        <div className="space-y-2">
          <p className="text-sm text-slate-600">Draft already exists for this period.</p>
          <form action={lockBillingRun}>
            <input type="hidden" name="id" value={existingDraft.id} />
            <button disabled={!rentPrice} className="rounded bg-slate-900 px-4 py-2 text-white">
              Lock billing run
            </button>
          </form>
          <Link className="inline-block text-sm underline" href={`/app/billing-runs/${existingDraft.id}`}>
            Open draft details
          </Link>
        </div>
      ) : null}
